...
// Modified by Thomas Bluemel circa 2016
//Updated to implement color changes in the spreadsheet
...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        /// <summary>
        /// If the zero-based column and row are in range, uses them to set
        /// the color of the selected cell and returns true.  Otherwise, returns false.
        /// </summary>
        /// <param name="col"></param>
        /// <param name="row"></param>
        /// <param name="c"></param>
        /// <returns></returns>
        public bool SetColor(int col, int row, Color c)
        {
            return drawingPanel.SetColor(col, row, c);
        }
        ...
        /// <param name="c"></param>
        /// <returns></returns>
        public bool GetColor(int col, int row, out Color theColor)
        {
            return drawingPanel.GetColor(col, row, out theColor);
        }

        /// <summary>
        /// Wipes all colors from the sheet
        /// </summary>
        public void ClearColors()
        {
            drawingPanel.ClearColors();
        }
        ...
        private class DrawingPanel : Panel
        {
            ...
            /// <summary>
            /// If the column and row are in bounds, Changes the color of a cell and returns true
            /// otherwise, returns false
            /// </summary>
            /// <param name="col">x index</param>
            ...
            public bool SetColor(int col, int row, Color c)
            {
                ...
                Address a = new Address(col, row);
                if (c == null || c == Color.FromName("White"))                  // if the color is null or white
                {
                    _colors.Remove(a);                                           // Remove the key value pair
                }
                else                                                            // if the color is anything else, do note only certain colors will be displayed
                {
                    _colors[a] = c;                                             // set the color at the address to the new color
                }
                Invalidate();
                return true;
            }
            ...
            /// <summary>
            /// If the column and row are in bounds, assigns the color of a cell to color argument and returns true
            /// otherwise, returns false
            /// </summary>
            /// <param name="col"></param>
            /// <param name="row"></param>
            ...
            public bool GetColor(int col, int row, out Color theColor)
            {
                if (InvalidAddress(col, row))                                   //If they entered an invalid cell
                {
                    theColor = Color.White;                                     //Set the color to white (i.e. clear) and return false
                    return false;
                }
                if(!_colors.TryGetValue(new Address(col, row), out theColor))   //if you cannot get the value out of the colors dictionary
                {
                    theColor = Color.White;                                     //set the color to white
                }
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                //Drawing the colors
                if (!(_colors == null) && _colors.Count != 0) // if color dictionary exists and is non empty
                {
                    foreach (KeyValuePair<Address, Color> address in _colors)     // for each pairing in the dictionary
                    {
                        Color bg = address.Value;                                 // get color at the addres
                        int x = address.Key.Col - _firstColumn;                   // offsets the address to fit in the currently displayed cells
                        int y = address.Key.Row - _firstRow;
                        int width = 2;                                            // width of the color outline
                        if (x >= 0 && y >= 0)                                     // if the address should be drawn
                        {
                            e.Graphics.FillRectangle(
                                new SolidBrush(address.Value),
                                new Rectangle(LABEL_COL_WIDTH + x * DATA_COL_WIDTH + (1 + width/2),          //fancy math to put the position of the border in the right spot
                                              LABEL_ROW_HEIGHT + y * DATA_ROW_HEIGHT + (1 + width/2),
                                              DATA_COL_WIDTH - (1+width),                                    // fancy math to make the border the right size
                                              DATA_ROW_HEIGHT - (1+width)));
                        }
                    }
                }
